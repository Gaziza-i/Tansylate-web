...
export default function Home() {
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-[#f0f0f0] py-4 px-6 rounded-b-2xl">
            {NAV_LINKS.map(({ label, id }, i) => (
              <a key={id} href={`#${id}`}
                className={`block py-3 text-sm text-[#3a3a3a] hover:text-[#1A1A1A] transition-colors${i < NAV_LINKS.length - 1 ? " border-b border-[#f0f0f0]" : ""}`}
  ...
  const FooterEditorial = () => {
    ...
    return (
      <footer id="contacts" className="bg-[#F2EAE1] border-t border-[#C8C3BB]/40">
